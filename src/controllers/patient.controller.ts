...
// Helper function to generate unique Patient IDs (e.g., PAT-12345678 or EMG-12345678)
const generateUniqueId = (prefix: 'PAT' | 'EMG'): string => {
...
export const registerNewPatient = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { 
      ...
      emergencyContactPhone, chiefComplaint, arrivalMode, arrivalTime 
    } = req.body;

    const nurseId = req.user?.id; // Extracted from Auth Token

    // Auto-generate the Patient ID
    const autoGeneratedPatientId = generateUniqueId('PAT');

    // 1. Create the Patient Record
    const { data: patient, error: patientError } = await supabase
      .from('patients')
      .insert([{
        patient_id: autoGeneratedPatientId,
        full_name: fullName,
    ...
    res.status(201).json({ 
      message: 'Patient fully registered and added to triage queue.', 
      patientId: patient.patient_id, // Return the generated ID to the frontend
      visit 
    });
  } catch (err) {
...
export const quickEmergencyRegistration = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Auto-generate a temporary ID for quick reg
    const autoGeneratedPatientId = generateUniqueId('EMG');
